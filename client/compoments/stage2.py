...
def render():
    ...
    number_url_each_query = st.number_input(
        ...
        min_value=1,
        value=hcache.get('number_url_each_query', default=10),
        help="Higher numbers will find more resources but take longer to process",
        on_change=lambda: hcache.set('number_url_each_query', number_url_each_query)
    )
    ...
    if hcache.exists('url_search'):
        ...
        with tab1:
            ...
            st.write(f"Unique websites: {df['url'].apply(lambda x: x.split('/')[2]).nunique()}")
            st.metric("Average results per query", round(len(df) / len(ss.list_query_engine_search), 1))
