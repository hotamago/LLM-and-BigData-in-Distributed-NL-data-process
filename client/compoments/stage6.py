...
def render():
    ...
    user_input_stage_6 = st.text_area(
        ...
        value=ss.hcache.get("user_input_stage_6", default=""),
        height=120,
    ...
    # Cache user input
    ss.hcache.set("user_input_stage_6", user_input_stage_6)
    ...
    if st.button("Generate Processing Script"):
        with st.spinner("Creating your processing script..."):
            ...
            columns_info = ss.hcache.get("columns_info", [])
    ...
    if ss.hcache.exists("final_script"):
        ...
        with st.expander("🔧 Advanced: View & Edit Script", expanded=False):
            st.warning(
                "⚠️ Advanced users only: Edit the script only if you understand Python programming."
            )
            ...
            with tab2:
                ...
                if st.button("💾 Save Script Changes"):
                    ss.hcache.set("final_script", edited_script)
            ...
            with st.spinner("Processing your data..."):
                try:
                    ...
                    try:
                        ...
                        initial_df = spark_no_access.read.parquet(
                    ...
                    script_to_run = ss.hcache.get("final_script", default="")
                    ...
                    try:
                        ...
                        final_df = spark_no_access.read.parquet(
                            cfg["hdfs"]["final_data"]
                        )
                        final_count = final_df.count()
                        progress.progress(1.0)
                        status.success(
                            f"Processing completed successfully! {final_count} records produced."
                        )
                        ...
                        st.error(
                            f"Processing completed, but could not read results: {str(e)}"
                        )
                    ...
                    st.info(
                        "Try simplifying your processing instructions or contact support."
                    )
    else:
        st.info(
            "Enter your processing instructions and click 'Generate Processing Script' to begin."
        )

    # Add a completion message at the end
    if ss.hcache.exists("final_script") and "final_df" in locals():
        st.success(
            "🎉 Congratulations! You've completed all steps of the data processing workflow."
        )
        st.write(
            "You can now download your results or explore them using the Data Query Tool."
        )
