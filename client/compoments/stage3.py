...
def render():
    ...
    content_button = st.button('Collect Content', key='get_content', 
    ...
    if content_button:
        ...
        try:
            ...
            st.error("We encountered a problem while collecting content. Please try again or contact support.")
    ...
    try:
        ...
        if rows_count > 0:
            ...
            with col2:
                ...
                avg_length = df_hdfs.select("content").rdd.map(lambda x: len(x[0]) if x[0] else 0).mean()
            ...
            with tab1:
                ...
                preview_df = df_hdfs.select("title", "url", "status_code").limit(10).toPandas()
                ...
                with st.expander("Example Content"):
                    ...
                    st.write("**Content:**")
                    st.write(sample["content"][:500] + "..." if len(sample["content"]) > 500 else sample["content"])
            
            with tab2:
                # Add download button for full data as CSV
                st.write("Download options for the collected content:")
                csv = df_hdfs.select("title", "url", "content", "status_code").toPandas().to_csv(index=False).encode('utf-8')
                st.download_button(
            ...
            # Continue button
            st.markdown("---")
            st.write("When you're satisfied with the collected content, continue to the next step:")
