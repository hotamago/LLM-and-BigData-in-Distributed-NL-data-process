...
def render():
    ...
    if hcache.exists("columns_info"):
        ...
        with tab1:
            ...
            if st.button("Add Field", key="add_field"):
                if new_field_name and new_field_desc:
                    columns_info.append({"name": new_field_name, "description": new_field_desc})
                    hcache.set("columns_info", columns_info)
            ...
            if columns_info:
                ...
                with col1:
                    if st.button("Update Field", key=f"update_field_{field_to_edit}"):
                        columns_info[field_to_edit] = {"name": edited_name, "description": edited_desc}
                        hcache.set("columns_info", columns_info)
        ...
        with st.expander("Advanced: Edit as JSON", expanded=False):
            ...
            if st.button("Update from JSON", key="update_columns_info"):
                try:
                    ...
                    hcache.set("columns_info", updated_info)
        ...
        # Continue button
        st.markdown("---")
        st.write("When you're satisfied with the extraction fields, continue to the next step:")
