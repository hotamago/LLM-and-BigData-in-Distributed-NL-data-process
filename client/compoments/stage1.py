...
def render():
    ...
    user_input = st.text_area(
        ...
        value=ss.hcache.get("user_input", default=""),
    ...
    number_queries_gen = st.number_input(
        "Number of queries to generate per topic:",
        min_value=1,
        value=ss.hcache.get("number_queries_gen", default=10),
    ...
    if st.button("Generate Queries", key="gen_query"):
        ...
        # Update cache
        ss.hcache.set("user_input", user_input)
        ss.hcache.set("number_queries_gen", number_queries_gen)
                ...
                list_user_query.append(
                    {
                        "user_input": query,
                        "num_queries": min(
                            max_batch, number_queries_gen - (_ * max_batch)
                        ),
                    }
                )
        ...
        with st.spinner("Generating queries..."):
            ...
            for i in range(len(list_user_query)):
                ...
                process_display.write(
                    f"Progress: {i + 1}/{len(list_user_query)} topics"
                )
                ...
                with st.spinner(f"Generating queries for: {query['user_input']}"):
                    while True:
                        try:
                            ss.list_query_engine_search_gen.extend(
                                gen_query(json.dumps(query))
                            )
                            break
    ...
    if "list_query_engine_search" in ss and ss.list_query_engine_search:
        ...
        if st.button("Edit Previous Queries", key="edit_old_queries"):
            ...
            if st.button("Save Changes", key="save_old_queries"):
                ...
                ss.hcache.set("list_query_engine_search", ss.list_query_engine_search)
    ...
    if "list_query_engine_search_gen" in ss and ss.list_query_engine_search_gen:
        ...
        st.markdown("---")
        st.write(
            "When you're ready to proceed to the next step, click the button below:"
        )
        if st.button("Use These Queries & Continue", key="confirm_queries"):
            # Update cache
            ss.hcache.set("list_query_engine_search", ss.list_query_engine_search_gen)
            # update stage
            ss.stage_process = 1
            st.rerun()
