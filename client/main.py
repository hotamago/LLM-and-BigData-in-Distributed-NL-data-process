...
if not users:
    ...
    setup_confirm = st.text_input(
        "Confirm Password", type="password", key="setup_confirm"
    )
...
if (
    ...
    and (current_time - ss.last_token_refresh) > 1800
):  # 30 minutes
...
if menu == "Home":
    # Global variables
    if ss.hcache.exists("list_query_engine_search"):
        ss.list_query_engine_search = ss.hcache.get(
            "list_query_engine_search", default=[]
        )
    ...
    with st.container():
        ...
        # Stage 1
        if (
            stage_process == list_stage_process[0]
        ):  # Stage 1: Generate queries for search engine api using one LLM
            from compoments.stage1 import render

            render()

        elif (
            stage_process == list_stage_process[1]
        ):  # Stage 2: Using search engine api to search url web
            from compoments.stage2 import render

            render()

        elif (
            stage_process == list_stage_process[2]
        ):  # Stage 3: From url send request to get content by multi spark node then each spark node send to hadoop to save
            from compoments.stage3 import render

            render()

        elif (
            stage_process == list_stage_process[3]
        ):  # Stage 4 Gen columns info for multi node for unified data processing
            from compoments.stage4 import render

            render()

        elif (
            stage_process == list_stage_process[4]
        ):  # Stage 5 Process data to get final data for analysis using spark and hadoop with multi node
            from compoments.stage5 import render

            render()

        elif (
            stage_process == list_stage_process[5]
        ):  # Stage 6: Final process with python code generate from LLM to process data and get result for analysis using spark and hadoop with multi node
